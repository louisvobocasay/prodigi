...
import {
  CoreEHelperRedisKeys,
  CoreHelperRedisService,
} from '../../../core-helper-redis';
...
export class CoreUsersCreateService extends CoreSharedService {
  ...
  createSessionKey(username: string, group: CoreEUsersGroups) {
  ...
  async createUserSession(
    ...
    expiryIn: number = CoreETime.A_DAY_IN_SECOND,
  ) {
    const sessionKey = this.createSessionKey(username, group);
